                ...
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span
                                    class="relative inline-flex bg-white items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700  border border-gray-300 cursor-default leading-5 dark:bg-gray-800 dark:border-gray-600">{{ $element }}</span>
                            </span>
                        @endif
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    ...
                                    <span aria-current="page">
                                        <span
                                            class="relative bg-primary text-white inline-flex items-center px-4 py-2 -ml-px text-sm font-medium   border border-gray-300 cursor-default leading-5 dark:bg-gray-800 dark:border-gray-600">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}"
                    ...
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next"
                            class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-red-300 active:text-gray-500 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:active:bg-gray-700 dark:focus:border-blue-800"
